...
public class Database<K, V> {
	 ...
	 * creating DB (setKeyBufferSize(int) and setValueBufferSize(int)). Default both
	 * sizes are equal to 10MB.
	 */
	...
	public static class Builder<K, V> {
		...
		private int keyBufferSize = 10000000; /* default size - 10MB */
		private int valueBufferSize = 10000000;
		 ...
		 * This buffers allow avoiding further costly allocations. So the size should be
		 * bigger than most of the keys in the DB because in case of every bigger key
		 * than size of the buffer there is required an additional allocation. Every
		 * thread has its own two buffers so the real size of used memory will be 2 *
		 * size * numberOfThreads. The default size is 10MB.
		 *
		 * @param size
		 *            Size of a buffer.
		 *
		 ...
		 * @since 1.1
		 */
		public Builder<K, V> setKeyBufferSize(int size) {
			if (size <= 0)
		 ...
		 * of values in the DB because in case of every bigger value than size of the
		 * buffer there is required an additional allocation. Every thread has its own
		 * buffer so the real size of used memory will be size * numberOfThreads. The
		 ...
		 * @since 1.1
		 */
		public Builder<K, V> setValueBufferSize(int size) {
			if (size <= 0)
				throw new IllegalArgumentException("Buffer size must be > 0");
