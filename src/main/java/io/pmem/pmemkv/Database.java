...
public class Database {
    ...
    public void getKeys(GetKeysBuffersCallback callback) {
        database_get_keys_buffer(pointer,
                (int kb, ByteBuffer k) -> callback.process((ByteBuffer) k.rewind().limit(kb)));
    }

    public void getKeysAbove(ByteBuffer key, GetKeysBuffersCallback callback) {
        ByteBuffer direct_key = getDirectBuffer(key);
        database_get_keys_above_buffer(pointer, direct_key.position(), direct_key,
                (int kb, ByteBuffer k) -> callback.process((ByteBuffer) k.rewind().limit(kb)));
    }

    public void getKeysBelow(ByteBuffer key, GetKeysBuffersCallback callback) {
        ByteBuffer direct_key = getDirectBuffer(key);
        database_get_keys_below_buffer(pointer, direct_key.position(), direct_key,
                (int kb, ByteBuffer k) -> callback.process((ByteBuffer) k.rewind().limit(kb)));
    }
    ...
    public void getKeysBetween(ByteBuffer key1, ByteBuffer key2, GetKeysBuffersCallback callback) {
        ...
        database_get_keys_between_buffer(pointer, direct_key1.position(), direct_key1, direct_key2.position(),
                direct_key2, (int kb, ByteBuffer k) -> callback.process((ByteBuffer) k.rewind().limit(kb)));
    }
    ...
    public void getAll(GetAllBufferCallback callback) {
        database_get_all_buffer(pointer, (int kb, ByteBuffer k, int vb, ByteBuffer v) -> callback
                .process((ByteBuffer) k.rewind().limit(kb), (ByteBuffer) v.rewind().limit(vb)));
    }
    ...
    public void getAbove(ByteBuffer key, GetAllBufferCallback callback) {
        ...
        database_get_above_buffer(pointer, direct_key.position(), direct_key,
                (int kb, ByteBuffer k, int vb, ByteBuffer v) -> callback.process((ByteBuffer) k.rewind().limit(kb),
                        (ByteBuffer) v.rewind().limit(vb)));
    }
    ...
    public void get(ByteBuffer key, GetKeysBuffersCallback callback) {
        ByteBuffer direct_key = getDirectBuffer(key);
        database_get_buffer_with_callback(pointer, direct_key.position(), direct_key,
                (int kb, ByteBuffer k) -> callback.process((ByteBuffer) k.rewind().limit(kb)));
    }
