...
extern "C" JNIEXPORT void JNICALL Java_io_pmem_pmemkv_Database_00024Builder_config_1from_1json
  (JNIEnv *env, jobject, jlong cfg, jstring jjson) {
    jboolean copySucceeded;
    const char* cjson = env->GetStringUTFChars(jjson, &copySucceeded);
    if (copySucceeded == JNI_FALSE || !cjson) {
      env->ThrowNew(env->FindClass(EXCEPTION_CLASS), "json string must be UTF-8 and cannot be null");
    } else {
